...
def get_news(limit:int = 100, offset:int = 0, since: str = "", to: str = "", order_by: str = "desc", with_tags: str = "", db: Session = Depends(get_db)):
    try:
        if with_tags:
            with_tags = [int(id) for id in with_tags.split(",")]
        data = GetNewsUsecase(db, limit, offset, since, to, order_by, with_tags)
